...
`,
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		...
		//CUSTOMIZED BY CYB
		Argument0 := req.Arguments[0]
		flag := "original"
		if len(req.Arguments[0]) == 47 {
			Argument0 = req.Arguments[0][0:46]
			flag = upCompressFlag[req.Arguments[0][46:]]
		}
		p := path.New(Argument0)
		...
		if flag != "original" {
			...
			//fmt.Fprintf(os.Stdout, "buffsize0:%d\n", buff.Len())
			newReader := bytes.NewReader(buff.Bytes())
			var mainReader io.Reader
			mainReader = io.NewSectionReader(newReader, 512, size)
			mainReader, err = WithMode(mode[flag], mainReader)
			if err != nil {
				return err
			}
			buff1 := bytes.NewBuffer([]byte{})
			n, err = io.Copy(buff1, mainReader)
			if err != nil {
				return err
			}
			/*fmt.Fprintf(os.Stdout, "buff1size:%d\n", buff1.Len())
			tr := tarc.NewReader(buff)
			fmt.Fprintf(os.Stdout, "buffsize1:%d\n", buff.Len())
			var hdr *tarc.Header
			for {
				hdr, err = tr.Next()
				if err == io.EOF {
			...
			res.SetLength(uint64(n))
			var buf bytes.Buffer
			hdr := &tarc.Header{
				Name: Argument0,
				Mode: 0600,
				Size: n}
			tw := tarc.NewWriter(&buf)
			if err := tw.WriteHeader(hdr); err != nil {
				return err
			}
			if _, err := tw.Write(buff1.Bytes()); err != nil {
				return err
			}
