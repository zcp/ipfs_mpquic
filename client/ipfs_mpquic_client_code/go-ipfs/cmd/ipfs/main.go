...
//CUSTOMIZED BY CYB
var compressType = []string{"snappy", "zlib", "zip", "zstd", "lz4", "lzma2", "lzo"}
var mode = map[string]CompressMode{"snappy": SnappyMode, "zlib": ZlibMode, "zip": ZipMode, "zstd": ZstdMode, "lz4": Lz4Mode, "lzma2": Lzma2Mode, "lzo": LzoMode}
...
//CUSTOMIZED BY CYB
func IndexContainStr(items []string, item string) int {
...
//CUSTOMIZED BY CYB
func IsContainStrs(items []string, item []string) bool {
	for _, eachItem := range item {
		if IsContainStr(items, eachItem) {
			return true
		}
	}
	return false
...
// 判断所给路径是否为文件夹 CUSTOMIZED BY CYB
...
// 判断所给路径文件/文件夹是否存在 CUSTOMIZED BY CYB
...
func mainRet() int {
	...
	//CUSTOMIZED BY CYB
	indexCompress := IndexContainStr(os.Args, "-u")
	if len(os.Args) > 3 && strings.ToLower(os.Args[1]) == "add" && indexCompress != -1 {
		//CUSTOMIZED BY CYB 20230226
		var compressMode = ""
		if os.Args[indexCompress+1] == "-d" {
			compressMode = os.Args[indexCompress+2]
		} else {
			compressMode = os.Args[indexCompress+1]
		} //CUSTOMIZED BY CYB 20230226
		if !IsContainStr(compressType, compressMode) {
			err := fmt.Errorf("invalid compressType")
			printErr(err)
			return 1
		}
		...
		if IsDir(filePath) {
			err := fmt.Errorf("%q folder does not support compression", filePath)
			printErr(err)
			return 1
		}

		if !Exists(filePath) {
			//printErr(errors.New("this file does not exist"))
			err := fmt.Errorf("%q does not exist", filePath)
			printErr(err)
			return 1
		}
		//CUSTOMIZED BY CYB 20230226
		if os.Args[indexCompress+1] == "-d" {
			...
			if err != nil {
				err := fmt.Errorf("create %q file error 1!", "delayaddlist")
				printErr(err)
				return 1
			}
			file, err := os.OpenFile(path.Join(filepath.Dir(exePath), "delayaddlist"), os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
			if err != nil {
				err := fmt.Errorf("create %q file error 2!"+path.Join(filepath.Dir(exePath), "delayaddlist"), "delayaddlist")
				printErr(err)
				return 1
			}
			defer file.Close()
			file.WriteString(strings.Join(os.Args, " ") + " " + time.Now().String() + "\n")
			return 0
		} //CUSTOMIZED BY CYB 20230226
		...
		if IsContainStr(compressType, compressMode) {
			...
			outName, err := c.WithMode(mode[strings.ToLower(compressMode)])
			if err != nil {
				printErr(err)
				return 1
			}
