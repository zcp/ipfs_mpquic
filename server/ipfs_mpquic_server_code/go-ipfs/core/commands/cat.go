...
//CUSTOMIZED BY CYB
var upCompressFlag = map[string]string{"0": "original", "1": "snappy", "2": "zlib", "3": "zip", "4": "zstd", "5": "lzma2", "6": "lzo", "7": "lz4"}
...
var CatCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		...
		ShortDescription: "Displays the data contained by an IPFS or IPNS object(s) at the given path.customized by cyb.",
	},
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		...
		//CUSTOMIZED BY CYB
		argmentTrans := func(arguments []string) ([]string, string) {
			anotherArguments := make([]string, len(arguments))
			flag := "original"
			for index, value := range arguments {
				if len(value) == 47 {
					anotherArguments[index] = value[0 : len(value)-1]
					flag = upCompressFlag[value[len(value)-1:]]
				} else {
					anotherArguments[index] = value
				}
			}
			return anotherArguments, flag
		}
		Arguments, flag := argmentTrans(req.Arguments)                                      //CUSTOMIZED BY CYB
		readers, length, err := cat(req.Context, api, Arguments, int64(offset), int64(max)) //CUSTOMIZED BY CYB
		if err != nil {
			return err
		}
		...
		*/
		...
		//CUSTOMIZED BY CYB
		if flag != "original" {
			reader, err = WithMode(mode[flag], reader)
			if err != nil {
				return err
			}
		}
