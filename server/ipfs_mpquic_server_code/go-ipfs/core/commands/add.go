...
var upCompressMode = map[string]string{"original": "0", "snappy": "1", "zlib": "2", "zip": "3", "zstd": "4", "lzma2": "5", "lzo": "6", "lz4": "7"}
...
const (
	...
	upCompressOptionName  = "upCompress" //CUSTOMIZED BY CYB
	delayAddOptionName    = "delayAdd"//CUSTOMIZED BY CYB 20230226
)
...
ipfs add -u lz4/zip/lzma2/zstd/lzop/snappy/zlib <path> (only for file) --customized by cyb 
ipfs add -u -d lz4/zip/lzma2/zstd/lzop/snappy/zlib <path> (only for file) --customized by cyb 
`,
	},
	...
	Options: []cmds.Option{
		...
		cmds.StringOption(upCompressOptionName, "u", "Algorithm for compressing files").WithDefault("original"), //CUSTOMIZED BY CYB
		cmds.StringOption(delayAddOptionName, "d", "Algorithm for compressing files delay"), //CUSTOMIZED BY CYB
	},
			...
			LOOP:
				for {
					...
					case out, ok := <-outChan:
						if !ok {
							if quieter {
								fmt.Fprintln(os.Stdout, lastHash) //CYB 20220210
							}
						...
						if len(output.Hash) > 0 {
							lastHash = output.Hash //CYB 20220210
							if quieter {
								continue
							}
							...
							if quiet {
								fmt.Fprintf(os.Stdout, "%s\n", output.Hash) //CYB 20220210
							} else {
								upCompress, _ := req.Options[upCompressOptionName].(string)
								fmt.Fprintf(os.Stdout, "added %s %s\n", output.Hash+upCompressMode[upCompress], cmdenv.EscNonPrint(output.Name))
							}
